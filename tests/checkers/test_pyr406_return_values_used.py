...
# test_pyr406_return_values_used_scope_resolution.py.
...
def test_no_false_positive_when_class_staticmethod_shares_name_with_unprotected_function() -> None:
    """A same-named class staticmethod's own protected return must not affect an unrelated module-level call.

    _protected_function_names collects names across the whole file, so "helper" enters that set via the
    staticmethod below even though it is never called bare. _is_protected_definition still re-checks the
    actual bound definition at the call site, which is the None-returning module-level function, so the call
    must not be flagged.
    """
    source = """
def helper() -> None:
