...
**Application to Pyrigor:** Keep the architecture, Rust/Python
implementation, CLI, rule model, configuration, documentation, and
workflow as simple as the real requirements allow. Simple does not mean
simplistic: complexity justified by correctness, security, performance,
...
testing, or human review against the specification.
...
tool or agent, human review, or other independent evidence.
...
regression in test effectiveness. Changes must not reduce the
established Mutmut mutation-score floor. The floor is a project-level
quality gate, not a per-change target to be optimised by weakening the
mutation test suite.
...
chronological development history in Git, issues, pull requests, or
other historical records rather than cluttering current code and
documentation with the story of how the current state was reached.
