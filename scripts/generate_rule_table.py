...
def _render_table(*, rows: list[RuleRow]) -> str:
    ...
    separator = f"| {'-' * id_width} | {'-' * title_width} | {'-' * enforced_width} |"
    lines = [
...
def _render_file(*, table: str) -> str:
    ...
    return (
        ...
        "by `scripts/generate_rule_table.py` -- do not edit\n"
        "this file by hand, it is overwritten on the next commit. See each rule's own "
