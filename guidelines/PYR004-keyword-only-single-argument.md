# PYR004 — Use keyword-only arguments even for single-parameter functions
...
adoptable rule from [PYR003](./PYR003-keyword-only-arguments.md), not
a stricter mode of it — a project may adopt PYR003 alone, PYR004
alone, or both.
...
[PYR003](./PYR003-keyword-only-arguments.md) exists primarily to
prevent argument-order confusion — a concern that
requires two or more parameters. A single-parameter function has no
order to get wrong. That is why PYR003 explicitly exempts it.
...
[PYR003](./PYR003-keyword-only-arguments.md) makes as one of several
reasons to adopt it — PYR004 isolates that argument on its own,
because it is the *only* one of PYR003's reasons that still applies
when there is only one parameter, and it is a real but comparatively
low-value protection: milder than the swap protection PYR003 provides
...
This asymmetry is exactly why PYR004 is a separate, opt-in rule
rather than folded into PYR003 itself: applying it universally risks
...
adopt PYR004 explicitly. A team that does not value it can adopt PYR003 alone
...
- Functions with two or more parameters — that is
  [PYR003](./PYR003-keyword-only-arguments.md)’s territory, not
  PYR004’s.
- The same exceptions [PYR003](./PYR003-keyword-only-arguments.md)
...
- [PYR003](./PYR003-keyword-only-arguments.md) — force keyword-only
  arguments for functions with two or more parameters. PYR003 and
  PYR004 are independent, adoptable alone or together.
